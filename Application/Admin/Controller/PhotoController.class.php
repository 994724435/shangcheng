...
class PhotoController extends CommonController {
	public function index(){
		$banner = M('Banner');
		$ban = $banner->where()->select();
		if(IS_POST){
            $idarr = I('post.idarr');
            $where = array('ban_id'=>array('in',$idarr));
            $res = $banner->where($where)->delete();
            if($res){
               echo "<script>window.location.href='/Admin/Photo/index';</script>";exit;
            }else{
                echo "<script>alert('删除失败');window.location.href='/Admin/Photo/index';</script>";exit;
            }
        }
		$data = array(
				'bp_id'=>$bp_id,
...
//				print_r($arr);die;
                $res = $banner->add($arr);
                if($res){
                	echo "<script>window.location.href='/Admin/Photo/index';</script>";
                }else{
